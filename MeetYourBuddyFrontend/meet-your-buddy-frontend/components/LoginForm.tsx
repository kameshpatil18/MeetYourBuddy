...
/* ─── shared field sx (mirrors RegisterForm) ─── */
const fieldSx = {
  '& .MuiInputLabel-root': {
...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://localhost:7152/gateway/auth/login', {
        method: 'POST',
      ...
      if (result.code === 1) {
        ...
        const userId = result?.data?.userId || result?.data?.id || 0
  ...
  return (
    ...
    >
      {/* ── Background atmosphere (mirrors RegisterForm) ── */}
      <Box sx={{
      ...
      <Box sx={{
        position: 'absolute', borderRadius: '50%', pointerEvents: 'none',
        width: 320, height: 320, top: '38%', right: '14%',
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            <Stack spacing={2.2}>
              ...
              <Button
                ...
                disabled={loading}
                endIcon={!loading && (
                  <ArrowForward sx={{ fontSize: '0.95rem !important' }} />
                )}
                sx={{
                  mt: 0.5,
                  height: 54,
